  ...
  const handleEvent = useCallback(
    (event: AGUIEvent) => {
      ...
      switch (event.type) {
        ...
        // ── Legacy (non-AGUI) event format ───────────────────────────────────
        // Emitted by invokeAgentCoreRuntime path (e.g. FormData file uploads).

        case 'init' as any: {
          const msgId = uuidv4()
          const msg = makeEmptyMessage(msgId, 'assistant')
          setMessages(prev => [...prev, { ...msg, isStreaming: true }])
          lastAssistantMsgIdRef.current = msgId
          startTextBuffer(msgId)
          setAgentStatus('thinking')
          setThinkingMessage('Processing…')
          break
        }
        ...
        case 'thinking' as any:
          setAgentStatus('thinking')
          setThinkingMessage((event as unknown as Record<string, string>).message || 'Processing…')
          break

        case 'response' as any: {
          const delta = (event as unknown as Record<string, string>).text ?? ''
          if (delta) {
        ...
        case 'tool_use' as any: {
          ...
          const tool: ToolExecution = {
            id: (e.toolUseId as string) || uuidv4(),
          ...
          appendTool(tool)
          setAgentStatus('thinking')
          setThinkingMessage(friendlyToolMessage(e.name as string))
          break
        }

        case 'tool_result' as any: {
          const e = event as unknown as Record<string, unknown>
          updateTool(e.toolUseId as string, t => ({
            ...t,
            toolResult: e.result as string,
            images: e.images as ToolExecution['images'],
            metadata: e.metadata as Record<string, unknown>,
            resultStatus: e.status as string,
            isComplete: true,
          }))
          break
        }
        ...
        case 'complete' as any:
          ...
          setMessages(prev =>
            prev.map(m =>
              m.id === lastAssistantMsgIdRef.current ? { ...m, isStreaming: false } : m,
            ),
          )
          setAgentStatus('idle')
          break
        ...
        case 'error' as any: {
          ...
          setMessages(prev => [
            ...prev,
            { ...errMsg, text: (event as unknown as Record<string, string>).message || 'Unknown error' },
          ])
          setAgentStatus('idle')
          break
        }
